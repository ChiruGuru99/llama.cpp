...
// Bottleneck stub test: replace one path with a minimum-work stub to find which
// side (producer or consumer) bounds throughput for a given shape. Build with
// -DSTUB_PRODUCER=1 or -DSTUB_CONSUMER=1, never both at once.
        ...
        if (use_mat) {
            ...
            for (int64_t unit = my_start; unit < base_units; unit += tiles_stride) {
                ...
                for (int64_t r = 0; r < ru_n; ++r) {
                    ...
                    const int64_t nsteps = (STUB_CONSUMER ? 1 : kps) * 2;
